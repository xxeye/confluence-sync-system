...
REM 多專案管理腳本啟動（Windows）
echo Confluence Sync System - 多專案管理腳本
echo ==========================================
echo.
...
@if errorlevel 1 (
  echo [錯誤] 找不到 Python。請先安裝 Python 3 並加入 PATH。
  echo.
  pause
  exit /b 1
)
...
@if not exist "configs.txt" (
  echo [警告] 找不到配置清單 configs.txt
  echo        請先建立 configs.txt，例如：
...
REM 統計 configs.txt 有效配置數量（跳過空行與 # 註解行；不依賴 findstr / 編碼）
...
REM 預設一律啟動 watch 模式
echo [運行] 啟動監聽模式（watch）...
echo.
@python multi_project_manager.py --config-list configs.txt --mode watch %*
@if errorlevel 1 (
  echo.
  echo [錯誤] multi_project_manager.py 運行失敗（watch）。請查看上方錯誤訊息。
  echo.
  pause
  exit /b 1
)
